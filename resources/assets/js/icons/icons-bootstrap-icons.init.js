...
async function fetchIcons() {
    try {
        const response = await fetch('assets/json/icons/bootstrap-icons.json');
...
async function displayIcons(filter = '') {
    ...
    filteredIcons.forEach(icon => {
        ...
        iconBox.innerHTML = `<i class="bi bi-${icon}"></i><strong>${icon}</strong>`;

        // Add click event to copy the icon format
        iconBox.addEventListener('click', () => {
            const iconFormat = `<i class="bi bi-${icon}"></i>`;
